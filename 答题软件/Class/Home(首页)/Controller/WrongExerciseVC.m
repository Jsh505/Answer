...
@interface WrongExerciseVC () <SWRevealTableViewCellDelegate,SWRevealTableViewCellDataSource>
{
    int _page;
}
...
@implementation WrongExerciseVC
...
- (void)loadData
{
    ...
    [PPNetworkHelper POST:@"/exam/wrong_list/" parameters:parametersDic hudString:nil success:^(id responseObject)
     {
         if (_page == 1)
         {
    ...
    SWCellButtonItem *item1 = [SWCellButtonItem itemWithTitle:@"取消收藏" handler:^(SWCellButtonItem *item, SWRevealTableViewCell *cell)
                               {
                                   ...
                                   [PPNetworkHelper POST:@"/exam/wrong_delete/" parameters:parametersDic hudString:@"删除中..." success:^(id responseObject)
                                    {
                                        // 从数据源中删除
